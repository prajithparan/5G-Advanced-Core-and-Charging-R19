...
TEST(NasCodec, DecodesRegistrationRequestNullSchemeSuci) {
    ...
    const std::vector<std::uint8_t> nas_pdu = {0x7e, 0x00, 0x41, 0x00, 0x00, 0x0d, 0x01, 0x99,
...
TEST(NasCodec, RejectsRegistrationRequestWithNonNullProtectionScheme) {
    ...
    std::vector<std::uint8_t> nas_pdu = {0x7e, 0x00, 0x41, 0x00, 0x00, 0x0d, 0x01, 0x99,
...
TEST(NasCodec, RejectsTruncatedAuthenticationResponse) {
    ...
    const std::vector<std::uint8_t> nas_pdu = {0x7e, 0x00, 0x57, 0x2d, 0x10, 0x01, 0x02, 0x03, 0x04};
...
TEST(NasCodec, EncodesSecurityModeCommandWithCorrectEnvelopeAndInnerLayout) {
    ...
    const auto nas_pdu = amf::nas::encode_security_mode_command(knas_int, ue_sec_cap, /*downlink_count=*/0);
...
TEST(NasCodec, DecodeSecurityModeCompleteAcceptsGenuineMessage) {
    ...
    const auto outcome = amf::nas::decode_security_mode_complete(knas_int, knas_enc, /*uplink_count=*/0,
                                                                  nas_pdu);
    ASSERT_TRUE(outcome.has_value());
...
TEST(NasCodec, DecodeSecurityModeCompleteRejectsTamperedMac) {
    ...
    const std::vector<std::uint8_t> plain_inner = {0x7e, 0x00, 0x5e};
    const auto ciphered =
        aka_crypto::nea2_apply(knas_enc, 0, 1, 0, plain_inner);
...
TEST(NasCodec, DecodeSecurityModeCompleteRejectsWrongSecurityHeaderType) {
    ...
    EXPECT_FALSE(amf::nas::decode_security_mode_complete(knas_int, knas_enc, 0, nas_pdu).has_value());
}
...
TEST(NasCodec, EncodesRegistrationAcceptWithCorrectEnvelopeAndDecryptsToExpectedInner) {
    ...
    const auto nas_pdu = amf::nas::encode_registration_accept(knas_int, knas_enc, /*downlink_count=*/1);
    ...
    const auto plain = aka_crypto::nea2_apply(knas_enc, /*count=*/1, /*bearer=*/1, /*direction=*/1, ciphered);
...
TEST(NasCodec, DecodeRegistrationCompleteRejectsWrongSecurityHeaderType) {
    ...
    EXPECT_FALSE(amf::nas::decode_registration_complete(knas_int, knas_enc, 1, nas_pdu).has_value());
}
...
std::vector<std::uint8_t> build_ul_nas_transport(const aka_crypto::NasIntKey& knas_int,
    ...
    std::vector<std::uint8_t> plain_inner = {
        ...
        0x25, 0x09, 0x08, 'i', 'n', 't', 'e', 'r', 'n', 'e', 't', // dnn IEI + len + label("internet")
    };
...
TEST(NasCodec, DecodeUlNasTransportRejectsNonN1SmPayloadContainerType) {
    ...
    std::vector<std::uint8_t> nas_pdu = {0x7e, 0x02,
                                         ...
                                         static_cast<std::uint8_t>((mac >> 16) & 0xff),
                                         static_cast<std::uint8_t>((mac >> 8) & 0xff),
                                         static_cast<std::uint8_t>(mac & 0xff), 0x02};
    nas_pdu.insert(nas_pdu.end(), ciphered.begin(), ciphered.end());
