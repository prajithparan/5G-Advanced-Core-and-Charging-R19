...
// A value-derived BIT STRING (MSB-first, matching make_zero_bit_string's own convention above) --
// used for GUAMI's AMFRegionID/AMFSetID/AMFPointer using this AMF's REAL configured identity
// (amf_region_id/amf_set_id/amf_pointer, nf-config-driven, ADR-0088) rather than
// build_ng_setup_response's own disclosed all-zero placeholder -- HandoverRequest's GUAMI is a
// mandatory IE describing THIS AMF to the target gNB, not a fixed lab constant.
...
// TS 23.502 §4.9.1.3.2. Runs on the SOURCE association's own thread -- unlike PathSwitchRequest's
// cold cross-association lookup, the UE this concerns is already this association's own
// auth_state.
//
...
// node type, matching extract_global_gnb_id's own scope); SourceToTarget/TargetToSource-
// TransparentContainer are relayed byte-for-byte, opaque, per the real spec's own "AMF doesn't
// need to understand this" design (both are plain OCTET STRING, TS 38.413 §9.3.1.31/§9.3.1.32);
// SecurityContext reuses the exact derive_kgnb/derive_nh call (NCC=0, chain position 0)
// PathSwitchRequest already established (ADR-0090) for the identical disclosed reason;
// UEAggregateMaximumBitRate is this lab's own fixed default (no real per-subscriber AMBR tracked
// anywhere in this project); PDUSessionResourceSetupListHOReq's own per-session transfer is real
// but uses build_placeholder_ho_request_transfer's own disclosed placeholder N3 address (see its
// own header comment). One real relay in flight per target gNB at a time
