...
three slices: **UDM** `Nudm_UEAU`'s 3 operations — `GetRgAuthData`, `GenerateAv` (real HSS
...
`UpdateRoamingInformation` (ADR-0215), and `Nudm_UECM`'s AMF non-3GPP-access registration group
(`Non3GppRegistration`/`GetNon3GppRegistration`/`UpdateNon3GppRegistration`, ADR-0216, mirrors the
already-built `amf-3gpp-access` group) — individual stubbed/missing operations within already-wired
files, not a structural gap.
