...
| 4 | Charging + TM Forum SID/BSS layer | CHF live-verified end to end for the charging lifecycle a PDU session actually goes through: `Nchf_ConvergedCharging_Create` (ADR-0044) and `_Release` (ADR-0046), both wired to real SMF triggers (N40 at session establishment/teardown) and confirmed via independent Prometheus counters on both NFs, including the 404 negative path. `docs/CHARGING_MAPPING.md` written, reviewed, and its resolvable TODOs closed; first real SID mapping slice live-verified: SUPI → TM Forum `Party`/`Individual` (`libs/bss-sid/`), built by CHF on every real charging event (ADR-0045). New standalone `bss/product-catalog` service (real TMF620 Product Catalog Management, its own ODA component) makes product/tariff definitions real configurable data for the first time, live-verified including real mTLS enforcement (ADR-0047) — not yet consulted by CHF's rating (a separate, larger scope). `Nchf_ConvergedCharging_Update` and the rest of the SID/BSS mapping table remain deferred |
