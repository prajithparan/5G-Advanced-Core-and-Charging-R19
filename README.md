...
| 4 | Charging + TM Forum SID/BSS layer | Stage 0/1 done + live-verified: new CHF NF (`Nnrf_NFManagement` registration), real `Nchf_ConvergedCharging_Create` wired to SMF at N40, triggered by a real PDU Session Establishment and confirmed via both NFs' own independent Prometheus counters (ADR-0044). `docs/CHARGING_MAPPING.md` written, reviewed, and its resolvable TODOs closed; first real SID mapping slice live-verified: SUPI → TM Forum `Party`/`Individual` (`libs/bss-sid/`), built by CHF on every real charging event (ADR-0045). Update/Release and the rest of the SID/BSS mapping table remain deferred (no real 3GPP data populated for those fields yet) |
