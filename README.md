...
persistence, now 29 of free5GC's ~42+ real TS 29.504 resources including AMF non-3GPP-access
...
GET+PUT+PATCH combining create-or-replace and RFC 7396 merge-patch), and policy-data
Operator-Specific Data (ADR-0114, genuinely distinct from the subscription-data-scoped sibling)), and UPF (task #107
...
`PATH_SWITCH_REQ`, the remaining ~13 of UDR's ~42+ real TS 29.504 resources (19 more now closed —
...
(ADR-0113), and policy-data Operator-Specific Data (ADR-0114), 29 of ~42+ total), ProSe's
PAnF-dependent
