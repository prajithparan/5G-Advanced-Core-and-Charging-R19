...
| 4 | Charging + TM Forum SID/BSS layer | CHF live-verified end to end for the charging lifecycle a PDU session actually goes through: `Nchf_ConvergedCharging_Create` (ADR-0044) and `_Release` (ADR-0046), both wired to real SMF triggers (N40 at session establishment/teardown) and confirmed via independent Prometheus counters on both NFs, including the 404 negative path. `docs/CHARGING_MAPPING.md` written, reviewed, and its resolvable TODOs closed; first real SID mapping slice live-verified: SUPI → TM Forum `Party`/`Individual` (`libs/bss-sid/`), built by CHF on every real charging event (ADR-0045). New standalone `bss/product-catalog` service (real TMF620 Product Catalog Management, its own ODA component) makes product/tariff definitions real configurable data, live-verified including real mTLS enforcement (ADR-0047) — and CHF now genuinely consults it: a real rating engine converts real seeded catalog data (10GB/$25 plan) into a real `GrantedUnit`, live-verified end to end from a real PDU session through to the actual HTTP response body (ADR-0048). Quota-consumption tracking/re-authorization is now real end to end too (ADR-0050, 7 stages): UPF measures real per-TEID usage in-kernel (eBPF/XDP) and sends a real unsolicited Sx Session Report Request over SMF's new persistent bidirectional PFCP peer; SMF decodes it and calls a real `Nchf_ConvergedCharging_Update`; CHF applies the reported usage and re-authorizes from the same rating engine; SMF pushes the re-authorized quota back into UPF's live datapath via a real Sx Session Modification — live-verified both under real timing pressure and with real headroom (zero premature quota-exhaustion reports across a 260-packet run). Real, disclosed gaps remain (no forwarding stop on quota exhaustion, no balance/wallet deduction, no VOLTH/VOLQU trigger differentiation — see ADR-0050's Stage 6 closing summary). The rest of the SID/BSS mapping table (Service, Resource, Customer, Agreement, ProductOffering, ProductPrice, AppliedCustomerBillingRate, CustomerBill, BalanceTopUp, Event and their TMF APIs) remains deferred |
...
bss/<service>/    Standalone TM Forum ODA-layer services (not 3GPP NFs, no NRF registration) --
                  e.g. bss/product-catalog, a real TMF620 Product Catalog Management service.
