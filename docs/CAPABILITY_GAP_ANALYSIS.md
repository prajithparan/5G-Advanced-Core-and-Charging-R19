...
| SMF | ~10-16x | `UpdateSMContext`: `PATH_SWITCH_REQ`/`_ACK` slice CLOSED (task #101, ADR-0092, real downlink FAR/GTP-U control-plane); **COUNT CORRECTED (ADR-0251)**: 22 of the 26 real N2SmInfoType values remain a stub -- SMF now implements 4 (PATH_SWITCH_REQ/_ACK, plus HANDOVER_REQ_ACK in ADR-0248 and HANDOVER_REQUIRED in ADR-0249). AMF's own N2 handover NGAP side is now closed (ADR-0095/ADR-0096), and the AMF->SMF relay wiring for handover-triggered PDU session resource re-setup is **CLOSED, ADR-0258** -- AMF really calls this endpoint now, pinned by `tests/integration/test_smf_handover_n2sminfo.cpp` |
