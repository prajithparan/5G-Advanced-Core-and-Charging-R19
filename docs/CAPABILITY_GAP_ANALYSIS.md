...
This project's UDR (`nfs/udr/src/main.cpp`) implements 50 real resource endpoints (49 real
...
the provisioned-data group, keyed by ueId alone),
...
ADR's own real, deliberate design). What's covered is solid; the real gap is breadth -- roughly 49
...
ADR-0136, now 49 as of ADR-0137 -- see below). This is well past free5GC's own ~42+ figure for
real `Nudr_DataRepository` resource types; the real, still-open gap from here is
...
taking UDR from 48 to 49 of free5GC's ~42+ real `Nudr_DataRepository` resource types. This is
well past free5GC's own ~42+ figure; the real, still-open work from here is surveying the
remainder of `TS29505_Subscription_Data.yaml` itself (`group-data/*`,
