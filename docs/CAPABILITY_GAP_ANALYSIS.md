...
This project's UDR (`nfs/udr/src/main.cpp`) implements 47 real resource endpoints (46 real
...
GET-only, genuinely NOT part of the provisioned-data group, keyed by ueId alone),
...
ADR's own real, deliberate design). What's covered is solid; the real gap is breadth -- roughly 46
...
ADR-0130, 44 as of ADR-0131, 45 as of ADR-0133, now 46 as of ADR-0134 -- see below). This is well
...
free5GC's ~42+ real `Nudr_DataRepository` resource types. This is well past free5GC's own ~42+
figure; the real, still-open work from here is surveying the remainder of
`TS29505_Subscription_Data.yaml` itself (`group-data/*`, `rangingsl-privacy-data`,
`ranging-slpos-data`, `5mbs-data`, and others), not chasing a shrinking comparison count.
