...
This project's UDR (`nfs/udr/src/main.cpp`) implements 63 real resource endpoints (62 real
...
no UDR resource change), now 62 as of ADR-0152 -- see below). This is well past free5GC's own
~42+ figure for real
...
`hss-sdm-subscriptions` remain deferred.
...
| UDR | ~2.5-10x | Resource-type breadth (62 of 42+ real TS 29.504 resources closed, past parity -- remainder of `group-data`, bare `/subscription-data/{ueId}`, remaining subscription nested sub-collections, real webhook delivery, and several genuinely-blocked resources remain, see UDR section above) |
