...
This project's UDR (`nfs/udr/src/main.cpp`) implements 48 real resource endpoints (47 real
...
ADR's own real, deliberate design). What's covered is solid; the real gap is breadth -- roughly 47
...
ADR-0130, 44 as of ADR-0131, 45 as of ADR-0133, 46 as of ADR-0134, now 47 as of ADR-0135 -- see
below). This is well past free5GC's own ~42+ figure for real `Nudr_DataRepository` resource types;
the real, still-open gap from here is
...
(`group-data/*`, `ranging-slpos-data`, `5mbs-data`, and others), not chasing a shrinking
comparison count.
