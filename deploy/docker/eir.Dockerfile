# Multi-stage build for nfs/eir. Mirrors deploy/docker/scp.Dockerfile -- see amf.Dockerfile's
...
WORKDIR /build
COPY --from=builder /build/build/nfs/eir/eir /build/eir

EXPOSE 7792/tcp 9480/tcp

ENTRYPOINT ["/build/eir"]
