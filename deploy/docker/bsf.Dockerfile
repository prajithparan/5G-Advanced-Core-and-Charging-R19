# Multi-stage build for nfs/bsf. Mirrors deploy/docker/nssf.Dockerfile -- see amf.Dockerfile's
...
WORKDIR /build
COPY --from=builder /build/build/nfs/bsf/bsf /build/bsf

EXPOSE 7789/tcp 9477/tcp

ENTRYPOINT ["/build/bsf"]
