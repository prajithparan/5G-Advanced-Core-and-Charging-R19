# Multi-stage build for nfs/udr. Mirrors deploy/docker/udm.Dockerfile -- see amf.Dockerfile's
# header comment for why this image does NOT generate its own lab PKI at start (must share the
# same root CA as nrf/amf/smf/udm; see deploy/docker/docker-compose.yml's pki-init service).
...
WORKDIR /build
COPY --from=builder /build/build/nfs/udr/udr /build/udr

EXPOSE 7781/tcp 9468/tcp

ENTRYPOINT ["/build/udr"]
