# Multi-stage build for bss/roaming-interconnect. Mirrors deploy/docker/product-catalog.Dockerfile
...
# bss/roaming-interconnect/src/main.cpp's own header comment), built the same way as one for
...
RUN cmake -S . -B build -G Ninja \
    ...
    && cmake --build build --target roaming-interconnect
...
WORKDIR /build
COPY --from=builder /build/build/bss/roaming-interconnect/roaming-interconnect /build/roaming-interconnect

EXPOSE 7788/tcp 9476/tcp

ENTRYPOINT ["/build/roaming-interconnect"]
