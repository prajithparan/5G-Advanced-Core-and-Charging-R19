# Multi-stage build for nfs/nsacf (ADR-0276). Mirrors deploy/docker/eir.Dockerfile -- see
...
RUN cmake -S . -B build -G Ninja \
    ...
    && cmake --build build --target nsacf
...
WORKDIR /build
COPY --from=builder /build/build/nfs/nsacf/nsacf /build/nsacf

EXPOSE 7797/tcp 9484/tcp

ENTRYPOINT ["/build/nsacf"]
