# Multi-stage build for nfs/smf. Mirrors deploy/docker/amf.Dockerfile -- see that file's header
...
WORKDIR /build
COPY --from=builder /build/build/nfs/smf/smf /build/smf

EXPOSE 7779/tcp 9466/tcp

ENTRYPOINT ["/build/smf"]
