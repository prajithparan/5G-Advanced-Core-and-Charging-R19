# Multi-stage build for nfs/udm. Mirrors deploy/docker/amf.Dockerfile/smf.Dockerfile -- see
# amf.Dockerfile's header comment for why this image does NOT generate its own lab PKI at start
# (must share the same root CA as nrf/amf/smf; see deploy/docker/docker-compose.yml's pki-init
# service).
...
WORKDIR /build
COPY --from=builder /build/build/nfs/udm/udm /build/udm

EXPOSE 7780/tcp 9467/tcp

ENTRYPOINT ["/build/udm"]
