# Multi-stage build for nfs/smsf. Mirrors deploy/docker/eir.Dockerfile -- see amf.Dockerfile's
...
WORKDIR /build
COPY --from=builder /build/build/nfs/smsf/smsf /build/smsf

EXPOSE 7793/tcp 9481/tcp

ENTRYPOINT ["/build/smsf"]
