# Multi-stage build for nfs/nef. Mirrors deploy/docker/bsf.Dockerfile -- see amf.Dockerfile's
...
WORKDIR /build
COPY --from=builder /build/build/nfs/nef/nef /build/nef

EXPOSE 7790/tcp 9478/tcp

ENTRYPOINT ["/build/nef"]
