# Multi-stage build for nfs/scp. Mirrors deploy/docker/nef.Dockerfile -- see amf.Dockerfile's
...
WORKDIR /build
COPY --from=builder /build/build/nfs/scp/scp /build/scp

EXPOSE 7791/tcp 9479/tcp

ENTRYPOINT ["/build/scp"]
