...
vcpkg_cmake_configure(
    ...
    OPTIONS
        ...
        -DONNX_ML=ON
        # P4.8/ADR-0074: real, disclosed vcpkg-overlay-port fix. vcpkg's own upstream
        # ports/onnxruntime/portfile.cmake prints "The port requires 'onnx' port build with CMake
        # option ONNX_DISABLE_STATIC_REGISTRATION=ON" but the vcpkg onnx port itself never sets
        # it -- confirmed by reading both portfiles directly. Without it, onnx/defs/schema.cc's
        # OpSchemaRegistry::map() runs its own lazy self-registration (a function-local static
        # SchemasRegisterer) which conflicts with onnxruntime's own explicit registration flow,
        # producing a real, reproducible "N schema were exposed... M were expected" assertion
        # failure the moment a real onnxruntime::Session is constructed (found via this ADR's own
        # unit tests, tests/conformance/test_ai_inference.cpp). This overlay port is the real fix
        # upstream's own warning names, not a guess.
