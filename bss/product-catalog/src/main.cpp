...
//
// Scope, approved before implementation: real ProductOffering/ProductOfferingPrice CRUD (Create/
// Get/List/Delete -- the real TMF620 PATCH-for-update and the /listener/* event-notification
// callbacks are deferred, not needed to prove the data model is real and usable) against a real,
// in-memory store. NOT yet wired to CHF -- CHF still returns an empty grant; making it actually
// consult this catalog to rate a charging event is a real rating engine, a separate, larger, not-
// yet-approved scope. NOT a GUI -- Phase 7 (JSON-schema-driven operator console, not started) is
// the eventual consumer of this service's real API; this turn only builds the API/data model it
...
// - In-memory only, no persistence across restarts (nfs/product-catalog/src/store.hpp's own
//   disclosure) -- same simplification as every NF's own store.
...
int main() {
    ...
    product_catalog::ProductOfferingStore offering_store(std::string(kSelfBase) + kApiRoot +
                                                          "/productOffering");
    product_catalog::ProductOfferingPriceStore price_store(std::string(kSelfBase) + kApiRoot +
                                                            "/productOfferingPrice");

    auto meter = sbi_core::get_meter("product-catalog");
    auto offering_create_counter =
        meter->CreateUInt64Counter("product_catalog_offering_create_total", "Total ProductOffering creates");
    ...
    server.add_route(
        "POST", std::string(kApiRoot) + "/productOffering",
    ...
    server.add_route(
        "GET", std::string(kApiRoot) + "/productOffering",
    ...
    server.add_route(
        "GET", std::string(kApiRoot) + "/productOffering/{id}",
        [&offering_store](const sbi_core::http2::Request& req) {
            ...
            if (!offering.has_value()) {
                return sbi_core::http2::problem_response(404, "Not Found",
                                                          "No ProductOffering " + id);
            }
            return sbi_core::http2::Response::json(200, json(*offering).dump());
        });

    server.add_route(
        "DELETE", std::string(kApiRoot) + "/productOffering/{id}",
        [&offering_store](const sbi_core::http2::Request& req) {
            const auto id = req.path_params.at("id");
    ...
    server.add_route(
        "POST", std::string(kApiRoot) + "/productOfferingPrice",
    ...
    server.add_route(
        "GET", std::string(kApiRoot) + "/productOfferingPrice",
